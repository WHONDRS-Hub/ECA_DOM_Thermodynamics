...
input = 'C:/Users/gara009/OneDrive - PNNL/Documents - Core Richland and Sequim Lab-Field Team/Data Generation and Files/ECA/FTICR/03_ProcessedData/Formularity/EC_Data_Processed_FTICR/'

github = 'C:/Users/gara009/OneDrive - PNNL/Documents/GitHub/ECA_DOM_Thermodynamics/'

# ====== Read in data ======
# Processed ICR Data
data = read.csv(list.files(pattern = "*EC_Data.csv", path = input, full.names = T), row.names = 1)
mol = read.csv(list.files(pattern = "*EC_Mol.csv", path = input, full.names = T), row.names = 1) %>% dplyr::select(MolForm)


# Fixing and Mapping sample names
colnames(data) = gsub('SIR.','SIR-',colnames(data))

data = merge(data, mol,  by = 'row.names', all = TRUE)
data <- data[!is.na(data$MolForm), ]
row.names(data) = data$Row.names 
data = data %>% dplyr::select(-Row.names,-MolForm)
mol <- mol[!is.na(mol$MolForm), , drop = FALSE]
# ===== Data cleaning ==============
# Factor creation
...
mol_clean = mol[-which(row.names(mol) %in% peaks.to.remove),, drop = FALSE]
...
cal = read.csv(list.files(pattern = "*Calibrated_Samples.csv", path = input, full.names = T),
               check.names = F) %>% 
  dplyr::select(final, Samples = samples)
...
total_peaks_data <- data.frame(Samples = names(number_of_peaks), Total_Peaks_with_MF_Form = number_of_peaks)

number_of_peaks_clean <- sapply(data_clean, function(column) sum(column != 0))
total_peaks_data_clean <- data.frame(Samples = names(number_of_peaks_clean), Total_Peaks_with_MF_Clean_Form = number_of_peaks_clean)

df = merge(total_peaks_data,total_peaks_data_clean, by = 'Samples', all = T)
df$Difference_Form = df$Total_Peaks_with_MF_Form- df$Total_Peaks_with_MF_Clean_Form

df2 = merge(df,factors, by = 'Samples', all = T)

write.csv(df2, paste0(github,'Formularity_Peaks.csv'), row.names = F)
