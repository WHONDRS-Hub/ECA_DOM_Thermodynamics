...
write.csv(df.stats,'Weighted_averages_for_molecular_properties_per_sample_Total_Formulas.csv', row.names = F)
...
ggplot(df_filtered, aes(x = site, y = Weighted_Avg_delGcoxPerCmol, fill = Treatment)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Dry" = "darkorange", "Wet" = "lightblue")) +  # Custom colors
  
  labs(x = "Location", y = "Intensity weighted avg Gibbs per C mol Total MF") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


ggplot(df_filtered, aes(x = site, y = Weighted_Avg_Lambda, fill = Treatment)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Dry" = "darkorange", "Wet" = "lightblue")) +  # Custom colors
  labs(x = "Location", y = "Intensity weighted avg Lambda Total MF") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
write.csv(df.medians, 'Medians_of_Weighted_averages_for_molecular_properties_per_site_and_treatment_Total_Formulas.csv', row.names = F)
