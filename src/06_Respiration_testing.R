...
cube_predictors <- c("cube_Median_AI_mod", "cube_Median_NOSC", "cube_Median_DBE",
                "cube_Median_delGcoxPerCmol", "cube_Median_delGcoxPerCompmol",
                "cube_Median_Lambda", "cube_Median_Extractable_NPOC_mg_per_kg")

predictors <- c("Median_AI_mod", "Median_NOSC", "Median_DBE",
                     "Median_delGcoxPerCmol", "Median_delGcoxPerCompmol",
                     "Median_Lambda", "Median_Extractable_NPOC_mg_per_kg")
...
# Transformed data worked better so only tested with cube root data
...
# Print model comparison results
print(model_comparison_REML)
print(model_comparison_default)
...
gam.check(gam_models_REML$cube_Median_AI_mod)
gam.check(cube_gam_models$cube_Median_AI_mod)
gam.check(gam_models_REML$cube_Median_NOSC)
gam.check(cube_gam_models$cube_Median_NOSC)
gam.check(gam_models_REML$cube_Median_DBE)
gam.check(cube_gam_models$cube_Median_DBE)
gam.check(gam_models_REML$cube_Median_delGcoxPerCmol)
gam.check(cube_gam_models$cube_Median_delGcoxPerCmol)
gam.check(gam_models_REML$cube_Median_delGcoxPerCompmol)
gam.check(cube_gam_models$cube_Median_delGcoxPerCompmol)

# ==== Export Summary Files =====
for (model_name in names(gam_models_REML)) {
  summary_file <- paste0("06_GAM_Results/GAM_Results/summary_REML_", model_name, ".txt")
  sink(summary_file)
  print(summary(gam_models_REML[[model_name]]))
  sink()
}
...
# ===== Interaction Plots ====
vis.gam(best_model, 
        view = c("cube_Median_62948_Final_Gravimetric_Moisture_g_per_g", 
                 "cube_Median_delGcoxPerCmol"),
        plot.type = "persp")
...
pdf(paste0(figure_path,"Figure_3_Interaction_plot.pdf"), width = 3000, height = 2250)
vis.gam(
  best_model, 
  view = c("cube_Median_62948_Final_Gravimetric_Moisture_g_per_g", "cube_Median_delGcoxPerCmol"), 
  plot.type = "contour", 
  color = 'heat',
...
pdf(paste0(figure_path,"Figure_3_Interaction_plot_color2.pdf"))
...
png(paste0(figure_path,"Figure_3_Interaction_plot.png"), width = 3000, height = 2250, res = 300)
...
png(paste0(figure_path,"Figure_3_Interaction_plot_color2.png"), width = 3300, height = 2250, res = 300)
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjust the left margin to be larger
