...
pretty_names <- colnames(cor_matrix) %>%
  stringr::str_remove("^cube_") %>%
  stringr::str_remove("^effect_") %>%
...
ggsave("Figures/Figure_S3_correlation_plot_A.png", pA, width = 10, height = 8, dpi = 300)
ggsave("Figures/Figure_S3_correlation_plot_A.pdf", pA, width = 10, height = 8, dpi = 300)
...
pB <- ggplot(lda_plot_data, aes(x = LD1, y = LD2, color = cluster, fill = cluster)) +
  stat_ellipse(level = 0.95, alpha = 0.2, linewidth = 1.2) +
...
ggsave("Figures/Figure_S3_LDA_discriminant_space.png", plot = pB, width = 10, height = 8, dpi = 300)
ggsave("Figures/Figure_S3_LDA_discriminant_space.pdf", plot = pB, width = 10, height = 8, dpi = 300)
...
for (i in seq_along(vars_to_plot)) {
  ...
  p_box <- ggplot(site_lda_data, aes(x = cluster, y = .data[[var_name]], fill = cluster)) +
    geom_boxplot(alpha = 0.7, outlier.alpha = 0.35, linewidth = 0.45) +
    scale_fill_manual(values = c("1" = "#74C0FC", "2" = "#FFD43B", "3" = "#FF8FAB")) +
    labs(
      x = NULL,
      y = ylab_wrapped
